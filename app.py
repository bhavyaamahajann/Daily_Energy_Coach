...
# ---------------------------------------------------------------------------
# Build the ENTIRE page as one single HTML blob.
# Streamlit st.write / st.markdown each create an isolated DOM wrapper,
# so we CANNOT open a <div> in one call and close it in another.
# ---------------------------------------------------------------------------

step = st.session_state.flow_step

# --- Screen HTML per step ---------------------------------------------------
if step == 1:
    screen_html = """
    <div class="screen">
    ...
    """
    phone_class = "phone-container dark-phone"
...
# --- Combine everything into ONE markup string ------------------------------
full_html = f"""
...
.phone-container {{width:390px;min-height:780px;background:#FDFBF7;border-radius:3rem;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);overflow-y:auto;position:relative;display:flex;flex-direction:column;box-sizing:border-box;}}
