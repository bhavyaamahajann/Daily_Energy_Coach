...
# Read query parameters for step navigation
params = st.query_params
if "step" in params:
    try:
        st.session_state.flow_step = int(params["step"])
    except ValueError:
        pass
...
# We join lines to prevent double-newline splitting in Streamlit Markdown parser
style_overrides = """
...
div[data-testid="stAppViewContainer"] {background: #0F0F10 !important;}
"""
st.markdown(f"<style>{style_overrides.replace(chr(10), ' ')}</style>", unsafe_allow_html=True)

# CSS stylesheet - stripped of comments and double newlines to prevent Streamlit text rendering
custom_css = """
@import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=Inter:wght@300;400;500;600;700&display=swap');
body {background-color: #0F0F10; color: #1c1917; font-family: 'Inter', sans-serif;}
.phone-wrapper {display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 2rem 1rem; box-sizing: border-box;}
.phone-container {width: 390px; height: 780px; background: #FDFBF7; border-radius: 3rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); overflow-y: auto; position: relative; display: flex; flex-direction: column; box-sizing: border-box;}
.phone-container::-webkit-scrollbar {display: none;}
.phone-container {-ms-overflow-style: none; scrollbar-width: none;}
.screen {display: flex; flex-direction: column; flex: 1; padding: 2.2rem 1.8rem; box-sizing: border-box;}
...
.btn-green-link {width: 100%; background: #4A7C59; color: #FFFFFF !important; text-decoration: none; border-radius: 1.25rem; padding: 1.1rem; font-weight: 600; font-size: 0.95rem; display: inline-flex; align-items: center; justify-content: center; gap: 0.6rem; box-sizing: border-box; transition: all 0.2s ease;}
...
.dark-phone {background: #18181B !important;}
.alert-card {background: #FDFBF7; border-radius: 2rem; padding: 2.2rem 1.8rem; margin-top: auto; margin-bottom: auto; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.3); text-align: center; color: #1C1917;}
...
"""
st.markdown(f"<style>{custom_css.replace(chr(10), ' ')}</style>", unsafe_allow_html=True)

# Render main phone frame
st.write('<div class="phone-wrapper">', unsafe_allow_html=True)

# Step 2 renders in a dark-tinted phone container
container_class = "phone-container dark-phone" if st.session_state.flow_step == 2 else "phone-container"
...
if st.session_state.flow_step == 1:
    ...
    # Screen 2: Morning Shield Propose (Morning)
    st.write("""
    <div class="screen">
        <div class="alert-card">
            ...
            <div class="action-list-card">
                ...
                <div style="font-size: 0.92rem; font-weight: 700; color: #1C1917; margin-bottom: 0.65rem;">
                    You have 6 meetings today.
                </div>
                <div style="font-size: 0.85rem; color: #52525B; margin-bottom: 0.65rem; font-weight: 600;">
            ...
            <a class="btn-green-link" href="?step=3" target="_self">
    ...
    # Screen 3: Calendar Protected (Calendar Protected)
    st.write("""
    <div class="screen">
